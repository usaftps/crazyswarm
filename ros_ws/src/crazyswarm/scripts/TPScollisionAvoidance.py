...
def main():
    ...
    # slightly smaller than the radii used for the collision avoidance 
    ...
    for _ in range(args.loops):
        ...
        # perturbations in Z. If we didn't add the perturbations, we'd get 
        # purely 2D movement that doesn't take advantage of the free space 
        # in the Z axis to reduce conflicts.
        #
        # A more robust solution would be to add some random bias direction 
        # to each robot's reference direction, so it will have a preferred 
        # altitude when far away from the goal, giving Z separation even 
        # when the starts and goals are all coplanar.
        #
        ...
        # Optimal assignment with sum of Euclidean distances objective 
        # guarantees no collisions when following straight-line trajectories 
        # IF the robots have no volume. If not, collisions may occur, but 
        # they are rare in practice if the start and goal positions are 
        # sufficiently dispersed and not collinear.
        #
        ...
        # minimizing the # maximum distance for any robot, which generally 
        # is more important in multi-robot applications. We could use e.g. 
